...
*/
	...
	function addTableIndex($strName, $strField, $strUniqueFlag) {
		...
		$this->index_table[$strName] = array($strField, $strNotUniqueFlag);
	}
...
//echo $strSQL;
	...
	function import_form() {
		...
		echo "<table class=\"adminlist\" id=\"exportlist\">\n";
		echo "<tbody>\n";
		echo "<tr><th>" . JText::_("IMPORT_FILE_LABEL"). "</th>";
...
<?php
		...
		echo "</td></tr>\n";
		echo "<tr><th>" . JText::_("IMPORT_TEXT_LABEL"). "</th>";
		echo "<td><textarea class=\"inputbox\" name=\"import_text\" cols=\"50\" rows=\"10\" id=\"import_text\"></textarea></td></tr>\n";
		echo "<tr><th/><td><input class=\"button\" type=\"button\" value=\"".JText::_("IMPORT_BUTTON")."\" onclick=\"submitbutton('import')\" /> ";
...
//echo "***".$rep[0]."***";
		switch (strtolower($rep[0])) {
			...
			case 'image':
				...
				$options = array(JHTML::_('select.option',  '', '- '. JText::_( 'SELECT_IMAGE' ) .' -' ));
...
*/
	...
	function _table_exists ($table) {
		$realTableName = $this->db->replacePrefix($table);
		$tableList = $this->db->getTableList();
		return in_array($realTableName,$tableList);
	}
	...
	function install() {
		...
		if ($this->_table_exists($this->table) === false) {
			...
			$strSQL = "CREATE TABLE `".$this->table."` (\n";
...
//echo strtoupper($row[1])."=".str_replace(" BINARY","",strtoupper($arrRep[0]))." && ".$row[2]."=".$arrRep[1]." && ".str_replace("'","",$row[4])."=".str_replace("'","",$arrRep[2])." && ".$row[5]."=".$arrRep[3]."<br/>";
					if ((strtoupper($row[1]) != str_replace(" BINARY","",strtoupper($arrRep[0]))) ||
						($row[2] != $arrRep[1]) ||
		...
		while (list($strName,$arrValue) = each($arrIndexFound)) {
			if (isset($this->index_table[$strName]) === true) {
				//Check modify
				$arrRep = $this->index_table[$strName];
//echo $arrValue[1]."=".$arrRep[1]." && ".implode(",",$arrValue[0])."=".implode(",",$arrRep[0])."<br/>";
				if (($arrValue[1]!=$arrRep[1]) || (implode(",",$arrValue[0])!=implode(",",$arrRep[0]))) {
					...
					if ($this->db->query()) {
						...
						$this->db->setQuery($strSQL);
						if ($this->db->query()) {
							echo "Modify index `".$strName."` on table `".$this->table."`.<br/>";
						} else {
							echo "Error: ".$this->db->getErrorMsg(true)."<br/>";
						}
					} else {
	...
	function uninstall() {
		if ($this->_table_exists($this->table) === true) {
			// Drop table
