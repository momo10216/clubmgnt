...
function display_birthdays($data, $details, $cols, $colcount, $cmobject, $params, $uri, $bdtext="") {
	$name = array();
	foreach($data as $row) {
		$id = array_pop($row);
		...
		if ($bdtext != "") {
			...
			switch ($params->get('column_next')) {
				case "1": //(Weekday)
					$birthday = " <span class=\"cmbirth_today_person_birthday\">(".substr(JHTML::date($birthday, "D"),0,2).")</span>";
					break;
				case "2": //(month/day)
...
if (count($data_today) > 0) {
	...
	$name = display_birthdays($data_today, $details, $cols, $colcount, $cmobject, $params, $uri, JText::_("Today"));
...
if (count($data_next) > 0) {
	...
	$name = display_birthdays($data_next, $details, $cols, $colcount, $cmobject, $params, $uri);
	echo "\t\t<div class=\"cmbirth_today_person\">".implode($params->get('delimiter'),$name)."</div>\n";
	echo "\t</div>\n";
}
echo "</div>\n";
